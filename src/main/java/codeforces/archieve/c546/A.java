package codeforces.archieve.c546;
...
public class A {
    public static void solve() {
        int[] knw = FS.readArray(3);
        int k = knw[0]; // price of first babana
        int n = knw[1]; // money
        int w = knw[2]; // amount of bananas
        int needToBorrow = 0;
        int amount = 0;
        ...
        needToBorrow = n - amount >= 0 ? 0 : Math.abs(n - amount);

        FS.pt.println(needToBorrow);
    }
